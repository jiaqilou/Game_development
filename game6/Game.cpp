...
void Game::LoadData() {
	...
	for (int i = 1; i <= 16; i++) {
		string s = "Assets/Coin/coin" + std::to_string(i) + ".png";
		LoadTexture(s);
	}
	for (int i = 1; i <= 10; i++) {
	...
	int back = 0;
	for (int i = 0; i  < 223 * 64/1024; i++) {
		Actor* a = new Actor(this);
		Actor* b = new Actor(this);
		Actor* c = new Actor(this);
		SpriteComponent* sb = NULL;
		if (back == 0) {
			a->SetPosition(Vector2(1280*i, 384));
			sb = new SpriteComponent(a, 100);
			sb->mParallax = 0.25;
			sb->SetTexture(GetTexture("Assets/Background/Sky_0.png"));
		}
		else if (back == 1) {
			b->SetPosition(Vector2(1280 * i, 384));
			sb = new SpriteComponent(b, 100);
			sb->mParallax = 0.25;
			sb->SetTexture(GetTexture("Assets/Background/Sky_1.png"));
		}
		else if (back == 2) {
			c->SetPosition(Vector2(1280 * i, 384));
			sb = new SpriteComponent(c, 100);
			sb->mParallax = 0.25;
			sb->SetTexture(GetTexture("Assets/Background/Sky_2.png"));
		}	
		AddSprite(sb);
		back++;
		if (back == 3) back = 0;
	...
	for (int i = 0; i < 223 * 64 / 1024; i++) {
		...
		if (back == 0) {
			...
			sb = new SpriteComponent(a, 100);
			sb->mParallax = 0.5;
			sb->SetTexture(GetTexture("Assets/Background/Mid_0.png"));
		}
		else if (back == 1) {
			b->SetPosition(Vector2(1280 * i, 384));
			sb = new SpriteComponent(b, 100);
			sb->mParallax = 0.5;
			sb->SetTexture(GetTexture("Assets/Background/Mid_1.png"));
		}
		else if (back == 2) {
			c->SetPosition(Vector2(1280 * i, 384));
			sb = new SpriteComponent(c, 100);
			sb->mParallax = 0.5;
			sb->SetTexture(GetTexture("Assets/Background/Mid_2.png"));
		}
		AddSprite(sb);
		back++;
		if (back == 3) back = 0;
	...
	for (int i = 0; i < 223 * 64 / 1024; i++) {
		...
		if (back == 0) {
			...
			sb = new SpriteComponent(a, 100);
			sb->mParallax = 0.75;
			sb->SetTexture(GetTexture("Assets/Background/Fore_0.png"));
		}
		else if (back == 1) {
			b->SetPosition(Vector2(1280 * i, 384));
			sb = new SpriteComponent(b, 100);
			sb->mParallax = 0.75;
			sb->SetTexture(GetTexture("Assets/Background/Fore_1.png"));
		}
		else if (back == 2) {
			c->SetPosition(Vector2(1280 * i, 384));
			sb = new SpriteComponent(c, 100);
			sb->mParallax = 0.75;
			sb->SetTexture(GetTexture("Assets/Background/Fore_2.png"));
		}
		AddSprite(sb);
		back++;
		if (back == 3) back = 0;
	...
	{	
		for (int i = 0; i < line.length(); i++) {
			...
			if (line.at(i) == 'A') {
				...
				block->SetPosition(Vector2(64 * b-32, 16 + aaa * 32));
				AddSprite(block->GetSprite());
			}
			else if (line.at(i) == 'P') {
				...
				for (int i = 1; i <= 10; i++) {
					string s = "Assets/Player/Run" + std::to_string(i) + ".png";
					mPlayer->GetSprite()->AddImage(GetTexture(s));
				}
				mPlayer->SetPosition(Vector2(64 * b - 32, 16 + aaa * 32));
				AddSprite(mPlayer->GetSprite());
			}
			else if (line.at(i) == 'O') {
				...
				for (int i = 1; i <= 16; i++) {
					string s = "Assets/Coin/coin" + std::to_string(i) + ".png";
					coin->GetSprite()->AddImage(GetTexture(s));
				}
				
				coin->SetPosition(Vector2(64 * b - 32,  aaa * 32));
				AddSprite(coin->GetSprite());
